...
> ⚠ **CE TOTAL EST PASSÉ DE 65 À 70, ET DEUX POINTS VIENNENT D'UNE ADDITION
> QUI N'ADDITIONNAIT PAS.** Les trois autres sont du travail fait.
>
> ```text
...
| 01 | ADR | **108 ADR**, chacune avec sa condition de révision |
...
| ~~1. « Annule » depuis le téléphone~~ | **FAIT** — ADR-105. La file sait désormais à QUI rendre ce qu'elle garde, sans rien gagner du pouvoir d'exécuter. ⚠ Et la propriété qui compte n'est pas « ça marche depuis le téléphone » : c'est qu'on défait **l'opération montrée**, jamais « la dernière » telle qu'elle sera au moment du oui. |
| **1. Un moteur STT local** (Phase 5) | le verrou réel de la voix, et le chemin le plus court vers « aussi simple que Siri ». `docs/02` veut « STT fonctionnel réseau coupé » ; la passerelle le refuse déjà s'il n'est pas local (ADR-103). Fiche `docs/04` obligatoire. |
