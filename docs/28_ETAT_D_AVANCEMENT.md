...
| 3 Les 10 outils restants | 12 % | **70 %** | 8,4 | **12 outils sur 15** ; +`calendar_create` (ADR-044) · `calendar_update` (ADR-045) · `file_search` (ADR-046) · `briefing_generate` (ADR-047, **A7 débloqué**) — 3 restants |
...
| Affirmation | Mesure |
|---|---|
| 12 outils sur 15 | `grep "id:" src/tools/*.ts` → `memory_add`, `memory_search`, `note_create`, `task_create`, `task_list`, `audit_query`, `task_complete`, `calendar_read`, `calendar_create`, `calendar_update`, `file_search`, `briefing_generate` |
...
| | |
|---|---|
| **La qualité de ce qui est fait** | 58 % ne dit pas si le noyau est solide. Neuf défauts majeurs ont été trouvés et corrigés par la mesure ; le dixième existe. |
| **La difficulté restante** | la Phase 5 (voix) est plus longue que la Phase 3, à poids presque égal. |
| **Le travail hors plan** | `docs/17` à `docs/27` — banc de défaillance, chaos, deux mondes — ne figurent dans aucune phase de `docs/02`. Onze documents et 149 tests de banc n'entrent pas dans les 42 %. |
...
| 01 | ADR | **47 ADR**, chacune avec sa condition de révision |
| 02 | Plan d'exécution | phases −1→2 franchies ; **Phase 3 aux 7/10** ; 4→7 ouvertes |
...
| **1. Les 3 outils restants** (Phase 3) | `reminder_create` et `system_status` sont libres ; **`web_search` est bloqué par le Data Firewall** (`docs/26 §4.5`). Un outil d'email débloquerait **A8**, `memory_forget` **C3**. |
...
```text
ÉTENDUE FONCTIONNELLE   ≈ 58 %     ce que Jarvis sait faire
...
Un projet à 58 % d'étendue et 78 % de preuve est exactement dans le bon ordre.
